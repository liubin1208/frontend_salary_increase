(async function () {
  ...
  function setHeroHTML(heroes) {
    doms.ul.innerHTML = heroes
      .map((h) => {
        let cname = h.cname;
        if (doms.txtKeyword.value) {
          const reg = new RegExp(doms.txtKeyword.value, 'ig');
          cname = cname.replace(reg, function (key) {
            return `<em>${key}</em>`;
          });
        }

        return `<li><a href="https://pvp.qq.com/web201605/herodetail/${h.ename}.shtml" target="_blank">
          <img
